...
// GB Filter ACC
static char* const GBFILTER_START_ADDR = reinterpret_cast<char* const>(0x74000000);
static char* const GBFILTER_READ_ADDR  = reinterpret_cast<char* const>(0x74000004);
    ...
    for(int j = 0; j < height; j++){
      ...
      for(int v = -1; v <= 1; v ++){
        for(int u = -1; u <= 1; u++){
          ...
          write_data_to_ACC(GBFILTER_START_ADDR, buffer, 4);
        }
      }
      read_data_from_ACC(GBFILTER_READ_ADDR, buffer, 4);
      ...
      *(target_bitmap + bytes_per_pixel * (width * j + i) + 2) = r;
      *(target_bitmap + bytes_per_pixel * (width * j + i) + 1) = g;
      *(target_bitmap + bytes_per_pixel * (width * j + i) + 0) = b;
    }
  }
